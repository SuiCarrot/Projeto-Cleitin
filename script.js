...
function criarMonstro(qtd, a, b) {
    ...
    for (let i = 0; i < qtd; i++) {
        ...
        let vidaMonstro = random(5, 8);
        let danoMonstro = random(3, 5);
...
function mortalKombat(a = 0) {
    ...
    if (personagens.jogador.vida > 0) {
        console.log(
                `Parabéns você conseguiu matar \x1b[36m${monstros[a].nome}\x1b[0m`,
            );
...
function ifGameOver(qtd, a, b) {
    ...
    for (let i = 0; i < qtd; i++) {
        ...
        mortalKombat(i);
        if (gameOver == true) {
            console.log(`Você morreu para um \x1b[36m${monstros[i].nome}\x1b[0m`);
            break;
        } 
    }
